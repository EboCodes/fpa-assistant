...
api.interceptors.request.use((config) => {
  const token = localStorage.getItem('fpa_token') || localStorage.getItem('ajala_token');
  if (token) config.headers.Authorization = `Bearer ${token}`;
  return config;
});

// Inline SVG Icons for clean, professional look (Zero AI slop / zero emojis)
const Icons = {
  Send: () => (
...
// Formatted response component with strict formatting
function FormattedText({ text }) {
  ...
  const flushList = () => {
    if (currentList.length > 0) {
      if (listType === 'ol') {
        elements.push(
          <ol key={`ol-${elements.length}`} className="doc-ol">
        ...
        elements.push(
          <ul key={`ul-${elements.length}`} className="doc-ul">
            {currentList.map((item, idx) => (
              <li key={idx}>{parseInline(item)}</li>
            ))}
          </ul>
  ...
  const parseInline = (str) => {
    ...
    while ((match = regex.exec(str)) !== null) {
      ...
      if (token.startsWith('**') && token.endsWith('**')) {
        ...
        parts.push(<code key={match.index} className="doc-code">{token.slice(1, -1)}</code>);
      } else if (token.startsWith('http')) {
        parts.push(
          <a key={match.index} href={token} target="_blank" rel="noopener noreferrer" className="doc-link">
            {token}
          </a>
        );
      }
  ...
  lines.forEach((line, index) => {
    ...
    if (trimmed.startsWith('### ')) {
      elements.push(<h4 key={index} className="doc-h4">{parseInline(trimmed.slice(4))}</h4>);
    } else if (trimmed.startsWith('## ')) {
      elements.push(<h3 key={index} className="doc-h3">{parseInline(trimmed.slice(3))}</h3>);
    } else if (trimmed.startsWith('# ')) {
      elements.push(<h2 key={index} className="doc-h2">{parseInline(trimmed.slice(2))}</h2>);
    } else {
      elements.push(<p key={index} className="doc-p">{parseInline(trimmed)}</p>);
    }
  });
      ...
      <div className="site-header-inner">
        <Link to="/" className="site-brand">
          ...
          <div className="brand-meta">
            ...
            <span className="brand-inst">The Federal Polytechnic, Ado-Ekiti</span>
          </div>
        </Link>

        <nav className="site-nav">
          <Link to="/" className={`site-nav-item ${location.pathname === '/' ? 'active' : ''}`}>
            Overview
          </Link>
          <Link to="/chat" className={`site-nav-item ${location.pathname === '/chat' ? 'active' : ''}`}>
            Student Helpdesk
          </Link>

          {/* KNOWLEDGE BASE IS COMPLETELY HIDDEN FROM PUBLIC USERS - ADMIN ONLY */}
          {user?.role === 'admin' && (
            <Link
              to="/knowledge-base"
              className={`site-nav-item admin-tab ${location.pathname === '/knowledge-base' ? 'active' : ''}`}
            >
              Knowledge Base
            </Link>
          )}
          ...
          {user ? (
            <div className="auth-status-wrap">
              {user.role === 'admin' && (
                <Link to="/admin" className={`admin-portal-link ${location.pathname === '/admin' ? 'active' : ''}`}>
                  Admin Panel
                </Link>
              )}
              <div className="user-indicator" title={user.email}>
                <span className="user-circle">{user.name.charAt(0).toUpperCase()}</span>
                <span className="user-shortname">{user.name.split(' ')[0]}</span>
              </div>
...
function Home() {
  ...
  const handleConsult = (question) => {
    navigate('/chat', { state: { initialPrompt: question } });
  };
  ...
  const serviceCategories = [
    ...
    },
    {
      ...
      sampleQuery: 'How do I apply for campus hostel accommodation?',
    },
    {
      code: 'ICT',
      ...
      sampleQuery: 'I cannot log in to my student portal account. What should I do?',
    },
  ];
    ...
    <main className="landing-layout">
      ...
      <section className="institutional-hero">
        ...
        <p className="hero-lead">
          Official institutional service assistant providing immediate, verified guidance on admissions, fee schedules, course registrations, examinations, and student services.
        </p>
        ...
        <div className="hero-cta-row">
          ...
          <div className="frequent-topics-wrap">
            ...
            <button className="topic-btn" onClick={() => handleConsult('What are the admission requirements for ND programmes?')}>
              ND Admission Requirements
            </button>
            <button className="topic-btn" onClick={() => handleConsult('How do I pay school fees?')}>
              School Fees & Remita
            </button>
            <button className="topic-btn" onClick={() => handleConsult('Where can I check my semester results?')}>
      ...
      <section className="directory-section">
        <div className="directory-header">
          ...
          <p>Select any category below to consult verified institutional guidelines or submit a query directly.</p>
        </div>

        <div className="directory-grid">
          {serviceCategories.map((cat) => (
            <article key={cat.code} className="directory-card" onClick={() => handleConsult(cat.sampleQuery)}>
      ...
      <section className="official-disclaimer-strip">
        <p>
          Official portal of The Federal Polytechnic, Ado-Ekiti (Ekiti State, Nigeria). For formal notices and direct circulars, reference the institutional website at <a href="https://fedpolyado.edu.ng" target="_blank" rel="noopener noreferrer">fedpolyado.edu.ng</a>.
        </p>
      </section>
    </main>
  );
...
function Login({ setUser }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Authentication failed. Verify your email and password.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main className="auth-canvas">
      <div className="auth-box">
        ...
        <h2>{mode === 'login' ? 'Institutional Sign In' : 'Register Student Account'}</h2>
...
function Chat({ user }) {
  ...
  const loadConversation = async (id) => {
    ...
    try {
      ...
      const formatted = (data.messages || []).map((m) => [
        ...
        { role: 'assistant', content: m.ai_response, messageId: m.id, intent: m.intent }
      ]).flat();
  ...
  const submitFeedback = async (messageId, rating) => {
    if (!messageId) return;
    try {
      await api.post('/api/chat/feedback', { messageId, rating });
  ...
  const send = async (queryText) => {
    ...
    try {
      const { data } = await api.post('/api/chat/message', { message, conversationId });
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          content:
            ...
            'The institutional service could not be reached. Please check your network connection and try again.',
        },
      ]);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="desk-viewport">
      ...
      {user && (
        <aside className={`desk-sidebar ${showHistory ? 'sidebar-open' : ''}`}>
          <div className="desk-sidebar-header">
            <h3>Saved Sessions</h3>
            <button className="btn-new-topic" onClick={startNewTopic}>
              <Icons.Plus /> New
            </button>
          </div>
          <div className="desk-sidebar-content">
            {conversations.length === 0 ? (
              <p className="sidebar-empty-msg">No saved sessions.</p>
            ) : (
              conversations.map((c) => (
                <div
                  key={c.id}
                  className={`session-item ${c.id === conversationId ? 'session-item-selected' : ''}`}
                  onClick={() => loadConversation(c.id)}
                >
                  <span className="session-title">{c.title || 'Session'}</span>
      ...
      <section className="desk-workspace">
        ...
        <div className="desk-topbar">
          <div className="desk-topbar-left">
            {user && (
              ...
              >
                {showHistory ? 'Hide Sessions' : 'Session History'}
              </button>
            )}
            <div className="desk-title-block">
              <h2>FPA Helpdesk Session</h2>
              <span className="desk-status-tag">Connected &middot; Federal Polytechnic, Ado-Ekiti</span>
            </div>
          </div>
          {conversationId && (
            <button className="btn-clear-session" onClick={startNewTopic}>
              + Start New Inquiry
            </button>
          )}
        </div>
        ...
        {!user && (
          <div className="guest-notice-strip">
            <span>Guest session active. <Link to="/login">Sign in</Link> to preserve your inquiry records.</span>
          </div>
        )}
        ...
        <div className="desk-thread">
          {messages.length === 0 ? (
            <div className="desk-empty-slate">
              ...
              <h3>How can the institution assist you today?</h3>
              <p>Inquire regarding admissions, fees, course registrations, examinations, or student affairs.</p>

              <div className="starter-grid-wrap">
                <span className="starter-label">Frequent administrative inquiries:</span>
                <div className="starter-queries-grid">
                  {starterQueries.map((q, idx) => (
                    <button key={idx} className="starter-query-button" onClick={() => send(q)}>
                      <span>{q}</span>
                      <Icons.ArrowRight />
                    </button>
                  ))}
            ...
            messages.map((m, idx) => (
              <div key={idx} className={`dialog-entry ${m.role === 'user' ? 'dialog-user' : 'dialog-helpdesk'}`}>
                <div className="dialog-speaker-badge">
                  {m.role === 'user' ? 'Student' : 'FPA Helpdesk'}
                </div>
                <div className="dialog-bubble">
                  ...
                  {m.role === 'assistant' && m.messageId && (
                    <div className="dialog-feedback-row">
                      <span className="feedback-query">Was this information accurate?</span>
                      {feedbackSent[m.messageId] ? (
                        ...
                        <div className="feedback-btn-group">
                          <button
                            ...
                            onClick={() => submitFeedback(m.messageId, 5)}
                          >
                            Yes, accurate
                          </button>
                          <button
                            className="btn-feedback-action"
                            onClick={() => submitFeedback(m.messageId, 1)}
                          >
                            Incomplete
                          </button>
                        </div>
          ...
          {loading && (
            <div className="dialog-entry dialog-helpdesk">
              <div className="dialog-speaker-badge">FPA Helpdesk</div>
              <div className="dialog-bubble loading-bubble">
                <span className="loading-pulse-text">Consulting institutional records...</span>
              </div>
            </div>
          )}
        ...
        <div className="desk-input-dock">
          ...
          >
            <input
              ...
              autoFocus
            />
            <button type="submit" className="btn-desk-send" disabled={loading || !input.trim()}>
              <span>Submit</span>
              <Icons.Send />
            </button>
          </form>
          <div className="desk-disclaimer-note">
            Official administrative assistant for The Federal Polytechnic, Ado-Ekiti. Urgent notices are published on <a href="https://fedpolyado.edu.ng" target="_blank" rel="noreferrer">fedpolyado.edu.ng</a>.
          </div>
        </div>
      </section>
    </div>
      ...
      <div className="kb-repository-inner">
        <header className="kb-repo-header">
          <div className="repo-breadcrumb">
            <Link to="/admin">Admin Panel</Link> / <span>Knowledge Base Repository</span>
          </div>
          <h1>Institutional Knowledge Base</h1>
          <p>
            Internal directory of institutional Q&amp;A records utilized by the FPA Assistant. (Administrator Access Only)
          </p>
        </header>
        ...
        <div className="repo-filters-bar">
          ...
          <div className="repo-categories-row">
            <button
              className={`repo-cat-btn ${selectedCategory === '' ? 'repo-cat-active' : ''}`}
            ...
            {categories.map((c) => (
              <button
                key={c.id}
                className={`repo-cat-btn ${selectedCategory === c.name ? 'repo-cat-active' : ''}`}
        ...
        {loading ? (
          <div className="repo-state-loading">Retrieving repository records...</div>
        ) : items.length === 0 ? (
          ...
          <div className="repo-cards-grid">
            {items.map((item) => (
              <article className="repo-entry-card" key={item.id}>
                <div className="entry-card-meta">
                  <span className="entry-cat-tag">{item.category_name}</span>
                  {item.source && <span className="entry-source-tag">Source: {item.source}</span>}
                </div>
                <h3 className="entry-card-question">{item.question}</h3>
...
function Admin({ user }) {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [kbRes, catRes, statRes] = await Promise.all([
      ...
      setAnalytics(statRes.data.analytics || null);
    } catch {
      // Error
    } finally {
      setLoading(false);
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingItem) {
        await api.put(`/api/admin/kb/${editingItem.id}`, form);
      } else {
      ...
      setFormError(err.response?.data?.error || 'Failed to update knowledge record.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id, question) => {
    if (window.confirm(`Confirm deletion of the following record?\n\n"${question}"`)) {
      try {
  ...
  const filteredItems = items.filter((i) => {
    ...
    if (categoryFilter !== 'all' && i.category_id.toString() !== categoryFilter) return false;
    return true;
  });
    ...
    <main className="admin-portal-layout">
      <div className="admin-portal-inner">
        ...
        <header className="admin-header-row">
          <div>
            <span className="admin-scope-badge">Institutional Administration</span>
            <h1>FPA Assistant Management</h1>
            <p>Knowledge repository maintenance, operational metrics, and institutional updates.</p>
          </div>
          <div className="admin-controls-group">
            <Link to="/knowledge-base" className="btn-view-repository">
              View Repository
            </Link>
            <button className="btn-create-entry" onClick={openCreateModal}>
        ...
        {analytics && (
          <section className="admin-stats-grid">
            <div className="stat-box">
              ...
              <span className="stat-detail">Student accounts</span>
            </div>

            <div className="stat-box">
              <span className="stat-label">Total Sessions</span>
              <span className="stat-value">{analytics.total_conversations}</span>
              <span className="stat-detail">Inquiry sessions</span>
            </div>

            <div className="stat-box">
              <span className="stat-label">Queries Processed</span>
              <span className="stat-value">{analytics.total_queries}</span>
              <span className="stat-detail">Helpdesk answers provided</span>
            </div>

            <div className="stat-box">
              <span className="stat-label">Active Knowledge Records</span>
              <span className="stat-value">{analytics.active_kb_entries}</span>
              <span className="stat-detail">Verified data entries</span>
            </div>
          </section>
        )}
          ...
          <table className="admin-grid-table">
            ...
            <tbody>
              {filteredItems.map((item) => (
                <tr key={item.id}>
                  <td>
                    <span className="tag-category-pill">{item.category_name}</span>
                  </td>
                  <td className="col-question-content">
                    <strong className="entry-q-title">{item.question}</strong>
                    <p className="entry-q-preview">{item.answer.slice(0, 110)}...</p>
                  </td>
                  <td className="col-source">{item.source || '—'}</td>
                  <td>
                    <span className={`status-badge ${item.status === 'active' ? 'badge-live' : 'badge-disabled'}`}>
                      {item.status}
                    </span>
                  </td>
                  <td className="col-date">{new Date(item.updated_at).toLocaleDateString()}</td>
                  <td className="col-actions">
                    <button className="btn-table-edit" onClick={() => openEditModal(item)}>
                      Edit
                    </button>
                    <button
          ...
          <div className="modal-veil" onClick={() => setShowModal(false)}>
            <div className="modal-dialog" onClick={(e) => e.stopPropagation()}>
              <div className="modal-dialog-header">
                <h3>{editingItem ? 'Edit Knowledge Base Entry' : 'Create Knowledge Base Entry'}</h3>
                <button className="btn-close-modal" onClick={() => setShowModal(false)}>
                  ✕
                </button>
              </div>

              <form className="modal-form-content" onSubmit={handleSave}>
                ...
                <div className="form-row-field">
                  <label>Official Answer (Markdown formatting supported) *</label>
                  <textarea
                    required
                    rows="6"
                ...
                <div className="form-row-field">
                  ...
                  >
                    <option value="active">Active (available in queries)</option>
                    <option value="inactive">Inactive</option>
                  </select>
                </div>
                ...
                <div className="modal-button-strip">
                  ...
                  <button type="submit" className="btn-modal-confirm" disabled={saving}>
                    {saving ? 'Saving...' : editingItem ? 'Save Updates' : 'Add Entry'}
                  </button>
                </div>
              </form>
            </div>
  ...
  return (
    <div className="fpa-shell">
      ...
      <Routes>
        ...
        {/* KNOWLEDGE BASE: STRICTLY ADMIN ACCESS ONLY */}
        <Route
          path="/knowledge-base"
          element={user?.role === 'admin' ? <KnowledgeBase user={user} /> : <Navigate to="/chat" replace />}
        />
